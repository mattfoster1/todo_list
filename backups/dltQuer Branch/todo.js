...
var retrieveAllData = function() {
	...
	for (s = 0; s < maxSerialNo; s++) { //will this work without defining a variable? surely === should become =
		...
		if (mainTodoArray[s]["name"] == undefined) {
			
			console.log("><><undefined><><");
			dltQuer++;
			//TASK: need to go ahead to all subsequent divs and reduce their 'name' by 1
			...
			if (dltQuer != 0) { 
					console.log("dltQuer = " + dltQuer);
					// mainTodoArray[s]["name"] -= 1;
					s1 = s1 - dltQuer;
				}
			...
			document.getElementById("id" + t).innerHTML = mainTodoArray[s]["name"] // NB eventually this can just be hidden or changed to a variable
